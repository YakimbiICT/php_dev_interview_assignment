...
class Imact_View_Render_Json extends Imact_View_Render_Abstract {

	private $methodMap =array ( "add" => array("POST"),
								"edit" => array("POST"),
								"random" => array("GET"),
								"read" => array("GET", "HEAD"),
								"delete" => array("DELETE"));
	...
	private $contentMap =array ("add" => array("required"=> array("id")),
			"edit" => array("required"=> array("id"),"param" => array("description", "title") ),
			"read" => array("param"=> array("id")),
			"random" => array(),
	...
	private function validMethod($action){
		...
		if(empty($action) || !in_array(self::$method, $this->methodMap[$action])){
			$this->status = 400;
			return false;
		}
	...
	private function invalidContentMsg($action){
		...
		$msg = 'You must you the "'.$method.'" method for the "'.$action.'" action'."\n";
		...
		foreach($this->contentMap[$action]['required'] as $key=>$param){
			$msg .=  $param;
			if(count($this->contentMap[$action]['required']) != ($key+1)) $msg .= ",";
		}
	...
	private function assignData($type, $action){
		...
		switch($type){
			case "err":
				...
				break;
			case "data":
				if(isset($this->data)) $response = array_merge($this->data, $response);
				break;
		}

		if(empty($response['status'])) $response['status'] = $this->status;
