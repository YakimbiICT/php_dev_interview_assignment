<?php
 ...
 class Imact_Model_Image_Imgur extends Imact_Model_Image_Base {
 		...
 		public function __construct($adaptor="Imact_Model_Adaptor_Mysql", $location=array()){
			...
			$this->diff = array( "datetime" ,"animated" ,"views" ,"bandwidth" ,"deletehash", "link",
								 "vote", "account_url", "bandwidth" , "ups", "downs", "score", "is_album" );
			$this->api = new Imact_Model_Adaptor_Imgur();
		...
		public function readFavorites($id=""){
			...
			foreach($data as &$row){
				...
				if(!empty($row['specifics'])){
					...
					if(is_array($extra_metadata))$row = array_merge($row, $extra_metadata );
					unset($row['specifics']);
		...
		private function rollAgain(&$dat, &$ind,&$selection,$details){
			...
			if( 	array_key_exists($ind, $selection)
					|| $dat[$ind]['is_album']
					|| $dat[$ind]['height'] > 1000 ){
