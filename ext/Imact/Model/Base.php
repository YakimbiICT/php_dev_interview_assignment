...
abstract class Base extends Core
{
    ...
    public function __construct($adaptor = "Imact_Model_Adaptor_Mysql",
            $location = array())
    {
    ...
    protected function query($params)
    {
        ...
        if (!empty($params) && is_array($params)) {
            ...
            if (isset($params['where']))
                $query .= $this->build($params['where'], "and");
            $query .= " ;";
    ...
    protected function build($field, $style = "input")
    {
        ...
        foreach ($field as $mapKey => $data) {
            ...
            if ($mapKey != $lastKey && $style == "input")
                $part .= ' , ';

            if ($mapKey != $lastKey && $style == "and")
                $part .= ' AND ';
        }
