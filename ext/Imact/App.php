...
class App extends Base
{
    ...
    public function __construct()
    {
        ...
        if (self::$input['format'] == "json") {
            parse_str(rawurldecode(\HttpResponse::getRequestBody()),
                    self::$input['data']);
        ...
        $class = __NAMESPACE__."\\Controller\\" . self::$input['controller'];
    ...
    private function parseUri()
    {
        ...
        if (empty($portions[1]) || !in_array($portions[1], $controllers)
                || count($portions) < 3) {
            self::$err[] = "Please use the form http://imact.lagnus.info/collage/{action}/{id}";

            self::$input['controller'] = "Favorite";
            self::$input['action'] = "read";
        } else {
